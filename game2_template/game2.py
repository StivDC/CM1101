...
def list_of_items(items):
    ...
    return newitems[:len(newitems) - 2]
...
def execute_take(item_id):
    ...
    for item in current_room["items"]:
        if item_id == item["id"] and total_weight(inventory) + item["mass"] <= 3.5:
            inventory.append(item)
            current_room["items"].remove(item)
            found = True
        elif total_weight(inventory) + item["mass"] > 3.5:
            print("You cannot take this as you would be carrying", total_weight(inventory) + item["mass"], "kg. This is more than 3.5kg, the max weight you can carry.")
            return
    for item in inventory:
        if item["id"] == "unknown":
...
def main():
    ...
    while True:
        ...
        # Vicotry condition 1, mystery item
