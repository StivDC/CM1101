...
def execute_take(item_id):
    ...
    for item in current_room["items"]:
        if item_id == item["id"] and total_weight(inventory) + item["mass"] <= 4:
            inventory.append(item)
            current_room["items"].remove(item)
            found = True
            print(total_weight(inventory))
            print(item_id)
        elif total_weight(inventory) + item["mass"] > 4:
            print("You are carrying", total_weight(inventory) + item["mass"], "kg. This is more than 4kg, the max weight you can carry.")
            return
    for item in inventory:
        if item["id"] == "unknown":
